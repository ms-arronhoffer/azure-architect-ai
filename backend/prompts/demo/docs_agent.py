...
def render_docs_agent_prompt(design_json: str, spec_json: str) -> str:
    return (
        ...
        "and the exact role it plays in the request flow (and notes that the running "
        "app shows this live in the Azure Activity Panel), features, prerequisites, "
        ...
        "- DEPLOYMENT.md: step-by-step Azure setup, `az deployment group create` commands, env var capture from Bicep outputs, local run instructions, and the `?mock=1` no-backend preview.\n"
        "- DEMO_SCRIPT.md: a presenter talk track timed to design.duration_minutes — "
        "an opening hook, a beat-by-beat walkthrough that EXPLICITLY points at the "
        "in-app Azure Activity Panel and the live architecture diagram, calling out "
        "what each Azure service is doing live (use design.live_activity as the spine), "
        "the single wow moment, and likely Q&A. Written so a first-time presenter can "
        "deliver a world-class demo without prep.\n"
