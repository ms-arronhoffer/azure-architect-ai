...
import type { SseEvent, Citation, BicepResult, CostEstimate, AdrRecord, ChatMessage, WorkloadContext, NetworkTopology, WafPillarResult, Mode } from "../types";
...
type ResultTab = "explanation" | "diagram" | "runbook" | "bicep" | "cost" | "adr" | "waf" | "network";
...
export default function ArchitecturePanel({ mode = "architecture", onRefine, onModeChange, workloadContext }: ArchitecturePanelProps) {
...
<script type="text/javascript" src="https://viewer.diagrams.net/js/viewer-static.min.js"></script>
  ...
  function handleFiles(files: File[]) {
    ...
    files.forEach(async (file) => {
      ...
      if (isImage) {
        ...
        try {
          const res = await fetch("/api/parse", { method: "POST", headers: { "Content-Type": "application/octet-stream", "X-Filename": file.name }, body: await file.arrayBuffer() });
          if (res.ok) {
  ...
  async function handleImprove() {
    ...
    setIsImproving(true);
    try {
      const res = await fetch("/api/improve", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text: requirements }) });
      if (res.ok) {
  ...
  async function handleGenerate() {
    ...
    await stream("/api/architecture", buildPayload([]), applyEvent);
    setStatusMsg("");
  }
