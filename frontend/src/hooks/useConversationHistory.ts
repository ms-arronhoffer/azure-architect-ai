import { useCallback, useEffect, useRef, useState } from "react";
import type { ConversationRecord, ChatMessage, Mode } from "../types";
import { apiPath } from "../config/api";
import { useAuth } from "../auth/AuthProvider";

const MAX_CONVERSATIONS = 50;

export type SaveStatus = "idle" | "saving" | "saved";

export function useConversationHistory() {
  const [conversations, setConversations] = useState<ConversationRecord[]>([]);
  const [saveStatus, setSaveStatus] = useState<SaveStatus>("idle");
  const savedTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const { getAccessToken } = useAuth();

  // Stable ref so callbacks below don't need getAccessToken in their dep arrays.
  const getTokenRef = useRef(getAccessToken);
  useEffect(() => { getTokenRef.current = getAccessToken; }, [getAccessToken]);

  async function authHeaders(extra: Record<string, string> = {}): Promise<Record<string, string>> {
    const token = await getTokenRef.current();
    return token ? { Authorization: `Bearer ${token}`, ...extra } : extra;
  }

  useEffect(() => {
    async function load() {
      try {
        const res = await fetch(apiPath("/api/conversations"), {
          headers: await authHeaders(),
        });
        if (!res.ok) return;
        const rows = await res.json();
        setConversations(
          rows.map((r: ConversationRecord & { structuredResult?: string | null }) => ({
            ...r,
            structuredResult: r.structuredResult
              ? JSON.parse(r.structuredResult as string)
              : undefined,
          }))
        );
      } catch {
        // silently ignore — offline or auth not yet ready
      }
    }
    load();
  }, []); // eslint-disable-line react-hooks/exhaustive-deps

  const upsert = useCallback(
    async (id: string, mode: Mode, messages: ChatMessage[], structuredResult?: unknown) => {
      if (messages.length === 0) return;
      const title = messages.find((m) => m.role === "user")?.content.slice(0, 60) ?? "Untitled";
      const now = Date.now();
      const existing = conversations.find((c) => c.id === id);
      const record: ConversationRecord = existing
        ? { ...existing, messages, updatedAt: now, ...(structuredResult !== undefined && { structuredResult }) }
        : { id, mode, title, createdAt: now, updatedAt: now, messages, ...(structuredResult !== undefined && { structuredResult }) };

      setConversations((prev) => {
        const filtered = prev.filter((c) => c.id !== id);
        return [record, ...filtered].slice(0, MAX_CONVERSATIONS);
      });

      setSaveStatus("saving");
      try {
        await fetch(apiPath("/api/conversations"), {
          method: "POST",
          headers: await authHeaders({ "Content-Type": "application/json" }),
          body: JSON.stringify({
            ...record,
            structuredResult:
              record.structuredResult !== undefined
                ? JSON.stringify(record.structuredResult)
                : null,
          }),
        });
        setSaveStatus("saved");
      } catch {
        setSaveStatus("idle");
      }
      if (savedTimerRef.current) clearTimeout(savedTimerRef.current);
      savedTimerRef.current = setTimeout(() => setSaveStatus("idle"), 2500);
    },
    [conversations] // eslint-disable-line react-hooks/exhaustive-deps
  );

  const remove = useCallback(async (id: string) => {
    setConversations((prev) => prev.filter((c) => c.id !== id));
    try {
      await fetch(apiPath(`/api/conversations/${id}`), {
        method: "DELETE",
        headers: await authHeaders(),
      });
    } catch {
      // best-effort
    }
  }, []); // eslint-disable-line react-hooks/exhaustive-deps

  const clear = useCallback(async () => {
    setConversations([]);
    try {
      await fetch(apiPath("/api/conversations"), {
        method: "DELETE",
        headers: await authHeaders(),
      });
    } catch {
      // best-effort
    }
  }, []); // eslint-disable-line react-hooks/exhaustive-deps

  const fork = useCallback(
    async (id: string, mode: Mode, messages: ChatMessage[]) => {
      const baseTitle =
        messages.find((m) => m.role === "user")?.content.slice(0, 55) ?? "Untitled";
      const now = Date.now();
      const record: ConversationRecord = {
        id,
        mode,
        title: `Fork: ${baseTitle}`,
        createdAt: now,
        updatedAt: now,
        messages,
      };

      setConversations((prev) => {
        const filtered = prev.filter((c) => c.id !== id);
        return [record, ...filtered].slice(0, MAX_CONVERSATIONS);
      });

      try {
        await fetch(apiPath("/api/conversations"), {
          method: "POST",
          headers: await authHeaders({ "Content-Type": "application/json" }),
          body: JSON.stringify(record),
        });
      } catch {
        // best-effort
      }

      return record;
    },
    [] // eslint-disable-line react-hooks/exhaustive-deps
  );

  return { conversations, saveStatus, upsert, remove, clear, fork };
}
